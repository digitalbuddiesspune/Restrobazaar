...
const SignInModal = ({ onClose, onSwitchToSignUp }) => {
  ...
  const handleSendOTP = async (e) => {
    ...
    setOtpLoading(true);
    try {
      const response = await authAPI.sendOTPForLogin(otpData.phone);
      if (response.success) {
        setOtpSent(true);
        startOtpTimer();
        setError('');
  ...
  const handleOtpLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await authAPI.verifyOTPAndLogin(otpData.phone, otpData.otp);
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    className="block w-full pl-10 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-1 focus:ring-red-500 focus:border-red-500 transition-all bg-white text-gray-900 placeholder-gray-400 text-center text-lg tracking-widest"
                    placeholder="000000"
                    value={otpData.otp}
                    onChange={handleOtpChange}
                    maxLength="6"
